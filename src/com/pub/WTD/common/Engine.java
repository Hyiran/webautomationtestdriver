...
import com.pub.WTD.util.MailBuilder;
...
public class Engine {
	...
	public Engine(HashMap<String, String> webDriverList, List<String> caseList,
		...
		PropertyConfigurator.configure(GlobalInfo.rootPath + sp + "conf" + sp
				+ "log4j.properties");
		...
		while(iter.hasNext()) {
			...
			String logFolder = GlobalInfo.rootPath + sp + "logs" + sp
	...
	private void runInterfaceCases(String caseName, SessionData sessionData,Logger logger
		...
		try {
			...
			String caseFolderString=GlobalInfo.rootPath + sp+ "cases" + sp +"interfaces";
...
//			}
			...
			for(Element e:tests){
				...
				for (Element el : paramElems) {
					...
					paramEntity.setRandom(isRandom.equalsIgnoreCase("true")?true:false);
	...
	private void runCommonCases(String caseName, SessionData sessionData,Logger logger
		...
		Class cl;
		try {
			cl = Class.forName("cases."+singleCase[1]+"."
					+ singleCase[0]);
		...
		BaseCase testObj = (BaseCase) strCtor
				.newInstance();
